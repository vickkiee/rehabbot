...
class RehabEnv(gym.Env):
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        if self.render_mode == "human":
            link_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "wrist_3_link")
            #mujoco.mjv_initGeom( self.viewer.user_scn.geoms[0], type=mujoco.mjtGeom.mjGEOM_SPHERE, size=[0.05, 0, 0], pos=self.data.xpos[link_id], mat=np.eye(3).flatten(), rgba=(0.5, 0, 0.5, 0.8) ) 
            #mujoco.mjv_connector( self.viewer.user_scn.geoms[0], type=mujoco.mjtGeom.mjGEOM_LINE, size=2.0, point1=self.prev_pos, point2=self.data.xpos[link_id] )
            #self.draw_cylinder(self.data.xpos[link_id], 0.03)
    ...
    def draw_cylinder(self, center, radius, color=(0.5, 0, 0.5, 0.8)):
        """Adds a sphere to the existing scene without clearing previous geoms"""
        # Get current geom count and maximum capacity
        current_count = self.viewer.user_scn.ngeom
        max_geoms = self.viewer.user_scn.maxgeom
    
        # Check if we can add another geom
        if current_count >= max_geoms:
            print(f"Warning: Reached maximum geoms ({max_geoms}). Sphere not drawn.")
            return
        mujoco.mjv_initGeom( self.viewer.user_scn.geoms[current_count], type=mujoco.mjtGeom.mjGEOM_CYLINDER, size=[radius, 0, 0], pos=center, mat=np.eye(3).flatten(), rgba=color )
        self.viewer.user_scn.ngeom += 1
